...
import { 
  getSavedSession, logoutUser, UserProfile, Cohort, 
  getStoredCohorts, isSupabaseConfigured 
} from '../lib/supabase';
...
export default function AbiHubApp() {
  ...
  // AUTHENTIFIZIERUNG & STUFEN-STATE
  const [authStatus, setAuthStatus] = useState<'checking' | 'unauthenticated' | 'needs_cohort' | 'authenticated'>('checking');
  ...
  useEffect(() => {
    ...
    if (session.user) {
      ...
      if (session.user.avatar) setUserAvatar(session.user.avatar);
      if (session.user.quote) setUserQuote(session.user.quote);

      if (session.cohort) {
        setCurrentCohort(session.cohort);
        setAuthStatus('authenticated');
      } else {
        setAuthStatus('needs_cohort');
      }
    } else {
  ...
  const handleAuthSuccess = (user: UserProfile, isNewRegistration?: boolean) => {
    setCurrentUser(user);
    ...
    const session = getSavedSession();
    if (session.cohort && !isNewRegistration) {
      setCurrentCohort(session.cohort);
      setAuthStatus('authenticated');
      notify(`Willkommen zurück, ${user.name}!`);
    } else {
      setAuthStatus('needs_cohort');
    }
  };

  const handleQuickDemo = (role: 'Ersteller' | 'Schüler') => {
    const defaultCohorts = getStoredCohorts();
    const activeCohort = defaultCohorts[0];
    const demoUser: UserProfile = {
      id: role === 'Ersteller' ? 'user_leon' : 'user_schueler',
      name: role === 'Ersteller' ? 'Leon Hillger' : 'Jonas Weber',
      email: role === 'Ersteller' ? 'stufensprecher@abi2026.de' : 'jonas.weber@schule.de',
      role: role,
      avatar: '🎓',
      quote: role === 'Ersteller' ? '„Nicht für die Schule, für das Abi lernen wir!“' : '„Dabei sein ist alles.“',
      cohortId: activeCohort.id,
      cohortName: activeCohort.name,
      schoolName: activeCohort.school,
    ...
    if (demoUser.quote) setUserQuote(demoUser.quote);
    setCurrentCohort(activeCohort);
    setAuthStatus('authenticated');
    notify(`Angemeldet als ${role === 'Ersteller' ? 'Stufensprecher (Ersteller)' : 'Schüler'}`);
  };

  const handleCohortConfigured = (cohort: Cohort, updatedUser: UserProfile) => {
    setCurrentCohort(cohort);
    setCurrentUser(updatedUser);
    setUserRole(updatedUser.role);
    setAuthStatus('authenticated');
    setShowCohortModal(false);
    notify(updatedUser.role === 'Ersteller' ? `Stufe "${cohort.name}" erfolgreich angelegt!` : `Stufe beigetreten!`);
  };
    ...
    return (
      <header className={`fixed top-0 inset-x-0 h-16 ${themeClasses.bgApp}/90 backdrop-blur-md z-40 border-b ${themeClasses.border} flex items-center justify-between px-4`}>
        <div>
          ...
          <p className="text-[11px] text-[var(--tw-text-muted)]">
            {currentCohort ? `${currentCohort.school} • ${currentCohort.name}` : 'Gymnasium Abi 2026'}
          </p>
        </div>
...
const BottomNavBar = () => (
  ...
  // 1. Ladestatus beim Prüfen der Session
  ...
  // 2. Nicht angemeldet: Zeige Authentifizierungs- & Registrierungs-Interface
  ...
  // 3. Angemeldet, aber noch keiner Stufe zugeordnet: Onboarding für Stufe anlegen / beitreten
  if (authStatus === 'needs_cohort' && currentUser) {
    return (
      <div style={themeStyles}>
        <CohortOnboarding
          currentUser={currentUser}
          onCohortConfigured={handleCohortConfigured}
          themeClasses={themeClasses}
        />
      </div>
    );
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="relative w-full max-w-xl my-8">
            <button
              ...
              className="absolute right-4 top-4 z-10 p-2 bg-slate-800/80 hover:bg-slate-700 text-slate-300 rounded-full transition"
            >
              <X className="w-5 h-5" />
            </button>
            <CohortOnboarding
                ...
                return (
                  <div key={post.id} className={`${themeClasses.bgCard} border border-orange-500/40 rounded-[20px] p-4 shadow-lg relative`}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      {(userRole === 'Ersteller' || userRole === 'Admin') ? (
                        <>
                          ...
                          <button onClick={() => setPosts(posts.filter(p => p.id !== post.id))} className="border border-red-500/30 text-red-400 hover:bg-red-500/10 text-[11px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition ml-2">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </>
                      ) : (
                        post.isSelectedForPrint && (
                          <div className="border border-amber-500/20 text-amber-500/50 text-[11px] font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5">
                            <Bookmark className="w-3.5 h-3.5 fill-amber-500/50" /> Im Abibuch
                          </div>
                        )
                      )}
                    </div>
          ...
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className={`${themeClasses.bgCard} border ${themeClasses.border} rounded-[24px] p-6 shadow-lg flex flex-col items-center relative overflow-hidden`}>
               ...
               <div className="flex gap-3 w-full mt-5">
                 ...
                 {userRole === 'Ersteller' && (
                 <button onClick={() => { navigator.clipboard.writeText(joinKey); notify('Kopiert!'); }} className="flex-1 bg-[#2C2622] border border-orange-500/30 text-orange-400 text-[11px] font-mono font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 hover:brightness-110 transition cursor-pointer"><Key className="w-3.5 h-3.5"/> {joinKey}</button>
               )}
               </div>
            </div>
            ...
            {/* STUFEN-VERWALTUNG & BEITRITTSCODE */}
            <div className={`${themeClasses.bgCard} border ${themeClasses.border} rounded-[24px] p-5 shadow-lg space-y-4`}>
              ...
              <div className="p-3 bg-[var(--tw-bg-elevated)] border border-[var(--tw-border-color)]/60 rounded-xl flex items-center justify-between">
                <div>
                  <div className="text-[10px] text-[var(--tw-text-muted)]">Stufen-Schlüssel (Einladungscode)</div>
                  <div className="text-xs font-mono font-bold text-amber-400 tracking-wider mt-0.5">{currentCohort?.joinCode || joinKey}</div>
                </div>
                <button 
                  onClick={() => {
                    navigator.clipboard.writeText(currentCohort?.joinCode || joinKey);
                    notify('Beitrittscode kopiert!');
                  }}
            ...
            <button 
              ...
              className="w-full bg-[#1A0A0A] border border-red-900/50 hover:bg-red-950/40 text-red-500 text-xs font-bold py-4 rounded-[20px] flex items-center justify-center gap-2 transition shadow-md"
            >
        ...
        {currentView === 'members' && (
          <div className="space-y-4 animate-fadeIn pb-10">
            ...
            <div className={`${themeClasses.bgCard} border ${themeClasses.border} rounded-[24px] p-5 shadow-lg`}>
               <div className="flex justify-between items-start mb-5">
                 <div>
                   <h2 className="text-lg font-bold text-[var(--tw-text-main)] leading-tight">{currentCohort?.school || 'Gymnasium Abi 2026'}</h2>
                   <p className="text-[11px] text-[var(--tw-text-muted)] mt-1">{currentCohort?.name || 'Abiturjahrgang 2026'} • {activeMembers.length} Mitschüler</p>
                 </div>
                 <span className="bg-purple-900/40 text-purple-300 border border-purple-700/50 text-[10px] font-bold px-3 py-1.5 rounded-lg">
                   {currentCohort?.year ? `Abitur ${currentCohort.year}` : 'Abitur 2026'}
                 </span>
               </div>
                ...
                {activeMembers.map(m => (
                  <div key={m.id} className={`${themeClasses.bgCard} border ${themeClasses.border} rounded-[20px] p-4 flex justify-between items-center shadow-sm`}>
                     ...
                     <div className="flex items-center gap-2">
                       ...
                       <div className="relative">
                         <button onClick={() => setOpenMemberMenuId(openMemberMenuId === m.id ? null : m.id)} className="p-2 hover:bg-[var(--tw-bg-elevated)] rounded-full transition">
                           <MoreVertical className="w-4 h-4 text-[var(--tw-text-muted)]" />
                         </button>
                         {openMemberMenuId === m.id && (
                           <div className={`absolute right-0 top-10 w-48 ${themeClasses.bgCardElevated} border ${themeClasses.border} rounded-xl shadow-xl z-50 overflow-hidden`}>
                             {userRole === 'Ersteller' && m.role === 'Schüler' && (
                               <button onClick={() => {
                                  setMembers(members.map(member => member.id === m.id ? {...member, role: 'Admin'} : member));
                             ...
                             {(userRole === 'Ersteller' || userRole === 'Admin') && m.role !== 'Ersteller' && (
                               <button onClick={() => {
                                  ...
                                  setOpenMemberMenuId(null);
                                  notify(`${m.name} wurde entfernt.`);
                               }} className="w-full text-left px-4 py-3 text-[11px] font-bold text-red-500 hover:bg-white/5 transition flex items-center gap-2 border-t border-[var(--tw-border-color)]/50">
                                 <Trash2 className="w-3.5 h-3.5" /> Aus Kurs werfen
                               </button>
                             )}
                           </div>
                         )}
